#!/bin/sh
# ============================================================================
# Lockstep comparison: pyjr100emu (reference) vs Verilator JR-100 system
# (CPU + VIA + decode), full-field including VIA state.
#
...
#   Set LOCKSTEP_CPU_ONLY=1 to run the CPU-only DUT and comparison.
#
...
if [ "${LOCKSTEP_CPU_ONLY:-0}" = "1" ]; then
    DUT_BIN="$REPO/sim/obj_dir/Vmb8861"
    DIFF_FLAGS="--cpu-only"
else
    DUT_BIN="$REPO/sim/obj_dir/Vjr100_core"
    DIFF_FLAGS=""
fi
...
exec python3 "$REPO/tools/trace_diff.py" \
    "$WORK/$NAME.ref.trace" "$WORK/$NAME.dut.trace" $DIFF_FLAGS \
