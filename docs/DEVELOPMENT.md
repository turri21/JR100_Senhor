...
| E | M2 | 表示パイプライン `rtl/jr100/jr100_video.sv` 実装 ✅（文字ジェネレータ4種のグリフ源、VRAM共有CGRAM=実機仕様対応。ブート後READY画面がPython表示モデルと**画素完全一致**。フレームタイミングは暫定でM2のHDMI出力時に実機仕様へ調整）。CPU実リセットシーケンス実装 ✅（`vector_reset`: FFFE/FFFF ベクタフェッチ+I=1、ハーネス `--vector-reset` で検証済み）。残: 合成コンテナ検証（実行中）→ `emu`/`CONF_STR`/`hps_io`/`ioctl_*` 接続 → 初回合成 → 実機で BASIC 画面表示 |
