...
| F | M3/M4 | **完了（2026-07-24）**: キーボード実機確認、`joystick_0`→`0xCC02`（受入12パターン）、OSD PRGロード（RTLストリーミングパーサ、受入7ファイル）、`.bas`は`tools/bas2prg.py`変換（OSD直接ロードは将来課題）、音声出力帯域制御（帯域内周期一致/帯域外無音+VIA継続）、CPU割込み/WAI経路のゴールデンベクタ検証（5シナリオ、Iフラグ退避順の実バグを検出修正）、拡張RAM 4000-7FFF（OSD・リセット時ラッチ）。カセットFSKはOSDロード置換方針どおり非実装 |
