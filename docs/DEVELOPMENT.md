...
| D | M1 | R6522 VIA（`rtl/jr100/jr100_via.sv`）とシステム統合（`rtl/jr100/jr100_core.sv`: アドレスデコード・ROM書き込み保護・拡張I/O）を実装 ✅。maze系6本を**VIA状態込み全フィールド**で一致確認。sound_scale.prg はCPU状態・メモリが13,900サンプル一致（タイマ位相のみ下記の既知モデル差）。BASIC ROM起動照合・表示・キーボード入力検証は未着手 |
...
- 方針: 計画書 §5.2 のとおり、VIA タイマ・IRQ の検証は専用のサイクル単位試験で行う。M1 の BASIC 起動照合がタイマ位相に依存して発散する場合は、Python 側へバス位置オフセットの導入を検討する（要判断）
