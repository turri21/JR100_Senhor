...
| F | M3/M4 | 実機確認済み: PS/2キーボードでのBASIC操作・フォント面切替。残: `joystick_0`→`0xCC02` 接続（emuまでは配線済み、`jr100_top`への引き込みが未実装）、OSDからのPRG/BASロード（PROGコンテナ解析・BasicLoader相当）、音声出力帯域制御（AGENTS.md §3.4）、入力・音声受入試験 |
